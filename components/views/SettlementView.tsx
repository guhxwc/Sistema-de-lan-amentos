...
export const SettlementView: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const settlementToSave = { 
        ...
        driver_phone: currentSettlement.driver_phone ? currentSettlement.driver_phone.trim() : '',
  ...
  const handleSendWhatsApp = () => {
    if (!currentSettlement.driver_phone) {
        alert('Por favor, cadastre o telefone do motorista.');
        return;
    }
    
    // Create text message
    let message = `*Acerto de Viagem - ${currentSettlement.date.split('-').reverse().join('/')}*\n`;
    message += `*Motorista:* ${currentSettlement.driver}\n\n`;
    
    if (currentSettlement.commissions.length > 0) {
        message += `*CRÉDITOS / COMISSÕES:*\n`;
        currentSettlement.commissions.forEach(c => {
            let detail = (c.origin || c.destination) ? `${c.origin || ''} x ${c.destination || ''}` : c.description || '-';
            message += `- ${detail}: ${formatCurrency(Number(c.value) || 0)}\n`;
        });
        message += `\n`;
    }
    
    if (currentSettlement.additions.length > 0) {
        message += `*OUTROS CRÉDITOS:*\n`;
        currentSettlement.additions.forEach(a => {
            message += `- ${a.description || '-'}: ${formatCurrency(Number(a.value) || 0)}\n`;
        });
        message += `\n`;
    }
    
    if (currentSettlement.discounts.length > 0) {
        message += `*DÉBITOS / DESCONTOS:*\n`;
        currentSettlement.discounts.forEach(d => {
            message += `- ${d.description || '-'}: ${formatCurrency(Number(d.value) || 0)}\n`;
        });
        message += `\n`;
    }
    
    message += `*RESUMO:*\n`;
    message += `Total Créditos: ${formatCurrency(totals.totalCredits)}\n`;
    message += `Total Descontos: ${formatCurrency(totals.totalDiscounts)}\n`;
    message += `*SALDO FINAL: ${formatCurrency(totals.finalBalance)}*\n`;
    
    if (currentSettlement.observations) {
        message += `\n*Obs:* ${currentSettlement.observations}`;
    }

    const encodedMessage = encodeURIComponent(message);
    const phoneNumber = currentSettlement.driver_phone.replace(/\D/g, ''); // Remove non-numeric
        ...
        <div className="max-w-5xl mx-auto space-y-8 pb-24">
            <Card>
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <Input label="Telefone Motorista (WhatsApp)" name="driver_phone" value={currentSettlement.driver_phone || ''} onChange={handleInputChange} placeholder="Ex: 5511999999999" />
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                     ...
                     <div className="flex flex-wrap gap-2 justify-end pt-4 border-t border-slate-200">
                        ...
                        <Button variant="outline" onClick={handleSendWhatsApp} className="flex items-center gap-2 text-green-600 border-green-600 hover:bg-green-50" disabled={!currentSettlement.driver_phone}>
