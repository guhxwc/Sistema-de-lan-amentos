...
export const ReceivablesView: React.FC = () => {
  ...
  const [editingFreight, setEditingFreight] =
    useState<ReceivableFreight | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("receivable_freights-realtime")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "receivable_freights" },
        (payload) => {
          fetchFreights();
          fetchAutocompleteData();
        },
    ...
    const inboxChannel = supabase
      .channel("inbox_ctes-realtime")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "inbox_ctes" },
        (payload) => {
          fetchInbox();
        },
      )
  ...
  const filteredFreights = useMemo(() => {
    ...
    return freights.filter((freight) => {
      ...
      if (selectedYear !== "Todos") {
        if (!freight.date || !freight.date.startsWith(selectedYear))
          return false;
      }
  ...
  const summaryData = useMemo(() => {
    return filteredFreights.reduce(
      (acc, freight) => {
  ...
  const handleSelectFromInbox = useCallback(
    async (cte: ProcessedCte) => {
      let parsedData: any = {};
      if (cte.xml_content) {
        try {
          parsedData = parseFiscalXml(cte.xml_content);
        } catch (e) {
          console.error("Erro ao fazer parse do XML do Inbox", e);
        }
      }

      setNewFreight({
        ...getInitialFreight(),
        cte: parsedData.cte || cte.cte_number || "",
        date: parsedData.date?.split("T")[0] || cte.emission_date || new Date().toISOString().split("T")[0],
        client: parsedData.client || cte.client_name || "",
        origin: parsedData.origin || cte.origin || "",
        destination: parsedData.destination || cte.destination || "",
        uf_origin: parsedData.uf_origin || "",
        uf_destination: parsedData.uf_destination || "",
        toll_value: parsedData.toll_value ?? "",
        cargo_value: parsedData.cargo_value ?? "",
        total_value: parsedData.total_value || cte.total_value || 0,
      });
      ...
      fetchInbox();
      alert(
        "Campos preenchidos com sucesso. Agora informe a data de vencimento.",
      );
    },
    [fetchInbox],
  );

  const handleDeleteFromInbox = useCallback(
    async (id: string) => {
  ...
  const handleAddFreight = useCallback(async () => {
    ...
    const freightToAdd = {
      ...
      uf_origin: newFreight.uf_origin
        ? newFreight.uf_origin.toUpperCase()
        : null,
      uf_destination: newFreight.uf_destination
        ? newFreight.uf_destination.toUpperCase()
        : null,
      toll_value:
        (newFreight.toll_value as any) === "" ? 0 : newFreight.toll_value,
      cargo_value:
        (newFreight.cargo_value as any) === "" ? null : newFreight.cargo_value,
      total_value:
        (newFreight.total_value as any) === "" ? 0 : newFreight.total_value,
      paid_value:
        (newFreight.paid_value as any) === "" ? 0 : newFreight.paid_value,
    };
  ...
  const handleDeleteFreight = useCallback(
    async (id: string) => {
  ...
  const handleUpdateFreight = useCallback(
    async (updatedFreight: ReceivableFreight) => {
      const freightToUpdate = {
        ...
        uf_origin: updatedFreight.uf_origin
          ? updatedFreight.uf_origin.toUpperCase()
          : null,
        uf_destination: updatedFreight.uf_destination
          ? updatedFreight.uf_destination.toUpperCase()
          : null,
        toll_value:
          (updatedFreight.toll_value as any) === ""
            ? 0
            : updatedFreight.toll_value,
        cargo_value:
          (updatedFreight.cargo_value as any) === ""
            ? null
            : updatedFreight.cargo_value,
        total_value:
          (updatedFreight.total_value as any) === ""
            ? 0
            : updatedFreight.total_value,
        paid_value:
          (updatedFreight.paid_value as any) === ""
            ? 0
            : updatedFreight.paid_value,
      };
      const { error } = await supabase
        .from("receivable_freights")
        ...
        <div className="w-full mx-auto space-y-6">
          ...
          {overdueCount > 0 && (
            <div className="p-4 bg-red-100 border-l-4 border-red-500 text-red-700">
              ...
              <p>
                Você tem {overdueCount} frete(s) vencido(s) com saldo pendente.
              </p>
            </div>
          )}
