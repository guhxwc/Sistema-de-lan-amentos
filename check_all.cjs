...
async function run() {
  const { data, error } = await supabase.from('receivable_freights').select('id, cte, total_value, cargo_value, created_at').order('created_at', { ascending: false }).limit(5);
  console.log("Latest Freights:", data);
}
run();
