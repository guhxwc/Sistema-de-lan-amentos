...
async function ensureLoaded(): Promise<void> {
  ...
  loadingPromise = (async () => {
    ...
    (icmsRes.data || []).forEach((r: any) => {
      icmsRatesCache![r.uf] = { uf: r.uf, internal_rate: Number(r.internal_rate), is_reduced_origin: !!r.is_reduced_origin };
    });

    rctrcCache = {};
    (rctrcRes.data || []).forEach((r: any) => {
      rctrcCache![`${r.origin_uf}_${r.destination_uf}`] = Number(r.taxa_base_percent);
    });
    ...
    seguroConfigCache = configRes.data
      ...
      : { id: 1, desconto_rctrc_percent: 60, rc_dc_percent: 0.015, franquia_percent: 10, franquia_minima: 1000 };
  })();
...
export function calculateIcms(originUf: string, destinationUf: string, baseValue: number): IcmsResult {
  const origin = (originUf || '').toUpperCase().trim();
  const destination = (destinationUf || '').toUpperCase().trim();
  const base = Number(baseValue) || 0;

  if (!origin || !destination) {
    return { aliquota: 0, valor: 0, motivo: 'UF de origem/destino não informada' };
  }

  // Exceção de negócio: frete PR → PR isento (subcontratação Convênio ICMS 25/90)
  if (origin === 'PR' && destination === 'PR') {
    return { aliquota: 0, valor: 0, motivo: 'Isento (PR → PR, subcontratação)' };
  }
  ...
  if (origin === destination) {
    ...
    return { aliquota, valor: (base * aliquota) / 100, motivo: `Alíquota interna ${origin}` };
  }
  ...
  if (isReducedOrigin && !destIsAlsoReduced) {
    return { aliquota: 7, valor: (base * 7) / 100, motivo: `Interestadual reduzida (${origin} → ${destination})` };
  }

  return { aliquota: 12, valor: (base * 12) / 100, motivo: `Interestadual padrão (${origin} → ${destination})` };
}
...
export function calculateSeguro(originUf: string, destinationUf: string, baseValue: number): SeguroResult {
  const origin = (originUf || '').toUpperCase().trim();
  const destination = (destinationUf || '').toUpperCase().trim();
  const base = Number(baseValue) || 0;
  const config = seguroConfigCache || { desconto_rctrc_percent: 60, rc_dc_percent: 0.015, id: 1, franquia_percent: 10, franquia_minima: 1000 };
...
export function getSeguroConfig(): SeguroConfig {
  return seguroConfigCache || { id: 1, desconto_rctrc_percent: 60, rc_dc_percent: 0.015, franquia_percent: 10, franquia_minima: 1000 };
}
...
export function calculateFreightBreakdown(
  ...
  const seguro = calculateSeguro(originUf || '', destinationUf || '', totalValue);
